...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3) Ask the model to analyze — STRICT JSON output for traceable claims
    const sys = (systemPrompt ? systemPrompt + "\n\n" : "") +
      "Тебе дан ПОЛНЫЙ ТЕКСТ научной статьи из PMC. " +
      (userQuestion
        ? "Ответь на вопрос специалиста, опираясь СТРОГО на переданный текст статьи. "
        : "Сделай подробный клинический разбор на русском. ") +
      "Верни ТОЛЬКО строгий JSON-массив (без markdown, без ```json, без комментариев, без преамбулы). " +
      "Каждый элемент массива — объект со следующими полями:\n" +
      "  - claim: string  — формулировка пункта на русском (то, что увидит читатель в выжимке);\n" +
      "  - quote: string  — точная ДОСЛОВНАЯ фраза(ы) из переданного текста, скопированная без изменений, на языке оригинала; если прямой подтверждающей фразы нет — пустая строка \"\";\n" +
      "  - location: string  — раздел статьи (Abstract / Introduction / Methods / Results / Discussion / Limitations / Conclusion и т. п.) или \"\" если не определимо;\n" +
      "  - type: \"direct\" — если claim подтверждён буквальной цитатой из quote; \"inferred\" — если это твой вывод/обобщение и прямой фразы в тексте нет.\n" +
      "Жёсткие правила: цитаты НЕ выдумывай и НЕ перефразируй; quote должен буквально встречаться в переданном тексте; если такой фразы нет — ставь type:\"inferred\" и quote:\"\". " +
      (userQuestion
        ? "Сначала идут пункты, прямо отвечающие на вопрос специалиста, затем — необходимый контекст. "
        : "Состав 6–14 пунктов: дизайн и выборка, методы, ключевые количественные результаты, ограничения/риски смещения, практические выводы для специалиста, применимость к российской/детской практике. ") +
      "PMID статьи: " + pmid + (pmcid ? `; PMCID: ${pmcid}` : "") + ".";
