...
// Быстрые действия (shorten/deepen/expand/merge/rewrite_scientific) + произвольный промпт.
...
const ACTION_PROMPTS: Record<string, string> = {
  shorten: 'Сократи текст обзора в 1.5–2 раза, сохрани все ключевые тезисы и ссылки [N].',
  deepen: 'Углуби текст: добавь патофизиологические детали, механизмы, нюансы клинической картины. Сохрани ссылки [N] и добавь новые из контекста материалов.',
  expand: 'Расширь и дополни обзор: добавь новые разделы или подразделы, охвати смежные темы из контекста материалов. Сохрани стиль.',
  merge: 'Объедини текущий текст обзора с дополнительным контекстом ниже, устрани дубли, сохрани логику и ссылки [N].',
  rewrite_scientific: 'Перепиши текст в строго научном стиле медицинского журнала: убери разговорные обороты, добавь точность формулировок, сохрани ссылки [N] и структуру.',
};
...
Не удаляй ссылки [N] на источники без явной причины.`;

    const userText = [
      title ? `Название обзора: ${title}` : '',
      materials_context ? `Контекст материалов:\n${materials_context}` : '',
