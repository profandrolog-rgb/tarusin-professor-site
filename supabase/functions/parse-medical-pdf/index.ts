...
МИКРОБИОТА ПО ОСИПОВУ (ГХ-МС микробных маркеров, «Хромато-масс-спектрометрия микробных маркеров», бланки НПФ «Медбиоспектр», отчёты Осипова Г.А.):
- Это lab-документ. document_type="lab".
- Каждый микроорганизм/маркер — отдельный элемент lab_results: analyte = название таксона ровно как в бланке (например «Clostridium propionicum», «Eubacterium/Cl.coccoides», «Streptococcus mutans», «Candida spp.», «Herpes», «Эндотоксин (ЛПС)», «Плазмалоген»).
- value — измеренное значение (обычно ×10^5 кл/г или ×10^6 кл/г); unit бери из бланка («кл/г», «×10^5 кл/г»); множители/степени НЕ пересчитывай, оставляй число как напечатано и укажи единицу с множителем.
- Колонка «Норма» — это референс: одиночное число N трактуй как ref_high=N, ref_low=0 и положи сырой текст в ref_text. Диапазоны разбирай по общим правилам выше.
- Если в бланке есть колонки «Норма» и «Отклонение/Превышение» — берётся именно «Норма», отклонение не записывай в value.
