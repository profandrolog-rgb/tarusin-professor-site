  ...
  try {
    ...
    if (sources.length === 0) {
      ...
      const ctx = sources.map((s, i) =>
        `[${i + 1}] PMID:${s.pmid} — ${s.title}\n${s.authors}. ${s.journal}. ${s.year}.` +
        `${s.doi ? ` DOI:${s.doi}` : ""}\n${s.abstract}`
      ).join("\n\n");
      const sys = (systemPrompt ? systemPrompt + "\n\n" : "") +
        "Тебе предоставлены аннотации статей из PubMed. Отвечай на русском, по существу клинического вопроса, опираясь на эти источники. " +
        "После утверждений ставь ссылки в формате [PMID:XXXXXX]. Если в источниках нет ответа — скажи прямо, какие данные отсутствуют.\n\n" +
