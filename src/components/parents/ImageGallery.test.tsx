...
describe("ImageGallery — выравнивание последнего ряда", () => {
  ...
  for (const n of [2, 3, 4, 5, 6]) {
    it(`${n} фото: контейнер использует flex-wrap + justify-center (последний ряд центрируется)`, () => {
      ...
      // Каждая фотокарточка должна иметь basis для 3-колоночной сетки на md
      const buttons = flex!.querySelectorAll("button");
      expect(buttons.length).toBe(n);
      buttons.forEach((btn) => {
        expect(btn.className).toMatch(/md:basis-\[calc\(33\.333%-0\.5rem\)\]/);
      });
    });
  }
});
