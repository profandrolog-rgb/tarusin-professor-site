...
function ValText({
  ...
  const badgeWidth = Math.max(90, Math.min(160, Math.max(...lines.map((line) => line.length)) * 7 + 18));
  const badgeHeight = lines.length > 1 ? 30 : 20;
  const badgeCx = cx + 100;
  const badgeY = bottomY - badgeHeight / 2;
  return (
    <g className="val" pointerEvents="none">
      <rect x={badgeCx - badgeWidth / 2} y={badgeY} width={badgeWidth} height={badgeHeight} rx={5} fill="#fff" fillOpacity={0.98} stroke={statusStroke(v.status)} strokeWidth={1} />
