...
import { Send, Plus, Trash2, Paperclip, X, Bot, User, Loader2, FileText, Image as ImageIcon, Zap, Brain, Users, Settings, Copy, FileDown, FileType2, FileCode2, Download, Mic, Square, Globe, ExternalLink, Folder, FolderPlus, FolderOpen, ChevronRight, ChevronDown, MoreVertical, Pencil, FolderInput, Search, Layers, Lock, BookmarkPlus, Sparkles, Printer, ZoomIn, Copyright, Volume2, VolumeX } from "lucide-react";
...
export default function Cabinet() {
  ...
  const illustrateMessage = async (msgIdx: number, text: string) => {
    ...
    const prompt =
      "Создай образовательную клиническую медицинскую иллюстрацию в стиле анатомического атласа/учебника " +
      "(как Netter, Sobotta): схематично, без фотореализма, без обнажённых людей — только анатомические структуры " +
      "(органы, сосуды, ткани в разрезе), educational medical textbook diagram, non-explicit, clinical, anatomical chart. " +
      "Минимум текста, подписи на русском.\n\n" +
      "ВАЖНО про стороны: используй анатомическую ориентацию пациента — " +
      "правая сторона ПАЦИЕНТА находится СЛЕВА от зрителя, левая сторона ПАЦИЕНТА — СПРАВА от зрителя " +
      "(как при осмотре врачом лицом к лицу). Подписи 'правый/правая' и 'левый/левая' " +
      "обязательно соотноси со стороной пациента, а не зрителя. Если в тексте указана сторона " +
      "(например 'левостороннее варикоцеле'), помести изображение этой патологии справа на картинке.\n\n" +
      "Ответ:\n" + clean.slice(0, 1800);
