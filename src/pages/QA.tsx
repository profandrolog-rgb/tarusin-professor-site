...
const QA = () => {
  ...
  const filtered = questions.filter(
    (q) =>
      ...
      (q.answer_text && q.answer_text.toLowerCase().includes(search.toLowerCase()))
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageMeta
        title="Вопросы и ответы — Проф. Тарусин Д.И."
        description="Ответы профессора Тарусина Д.И. на вопросы пациентов. Задайте свой вопрос."
        path="/qa"
      />
      ...
      <header className="bg-primary text-primary-foreground py-12 md:py-20">
        <div className="container mx-auto px-4">
          ...
          <h1 className="text-3xl md:text-5xl font-bold mb-4">Вопросы и ответы</h1>
          <p className="text-lg md:text-xl text-primary-foreground/80 max-w-2xl">
            Ответы профессора Тарусина Д.И. на вопросы пациентов и их родителей
          </p>
        </div>
      </header>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground">
                {search ? "Ничего не найдено по вашему запросу" : "Пока нет опубликованных вопросов"}
              </p>
            </div>
          ) : (
                  ...
                  <AccordionTrigger className="text-left hover:no-underline py-5">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {q.author_name} • {new Date(q.created_at).toLocaleDateString("ru-RU")}
                      </p>
                    </div>
                  </AccordionTrigger>
