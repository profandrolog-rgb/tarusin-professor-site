  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Профессионалы, с которыми я работаю. Абсолютное доверие. Кристальная честность    
            </p>
          </div>
          {isAdmin && <Button onClick={() => setIsFormOpen(true)} className="gap-2">
        ...
        {isLoading ? <div className="text-center py-12 text-muted-foreground">
            Загрузка...
          </div> : members.length === 0 ? <div className="text-center py-12 text-muted-foreground">
            Специалисты пока не добавлены
          </div> : <div className="space-y-0">
            {members.map((member, index) => <TeamMemberCard key={member.id} member={member} isAdmin={isAdmin} isFirst={index === 0} isLast={index === members.length - 1} onEdit={() => handleEdit(member)} onDelete={() => deleteMutation.mutate(member.id)} onMoveUp={() => moveMutation.mutate({
          id: member.id,
          direction: "up"
        })} onMoveDown={() => moveMutation.mutate({
          id: member.id,
          direction: "down"
        })} />)}
          </div>}
