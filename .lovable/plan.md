## Правка №1 — Ускорение открытия протокола визита (`/admin/visits/:id`)

Статус: запланировано, не выполнено.

**Причина (по итогам диагностики 29.07.2026):** база и данные ни при чём (протокол ~772 байта, БД здорова). Задержка складывается из: обновления токена сессии, 4 последовательных round-trip через прокси `api.tarusin.pro` (TTFB 280–470 мс против 105–160 мс напрямую) и тяжёлого lazy-чанка страницы.

**Что сделать:**

1. **Динамический импорт PDF-библиотек.** `src/lib/exportPdf.ts` тянет `jspdf` + `html2canvas` статически, из-за чего они попадают в чанк страницы. Перевести на `await import(...)` внутри `exportNodeToPdf`, чтобы грузились только по клику «Скачать PDF».
2. **Параллельные запросы при открытии.** В `src/pages/AdminPatientVisitDetail.tsx` запрос визита и запрос соседних визитов (`siblings`, limit 50) выполняются последовательно — объединить в `Promise.all`.
3. **Ленивая подгрузка вспомогательных блоков.** `PdfBatchUpload`, `ProtocolPrintLayout` и диалог предпросмотра сделать `lazy()` — они не нужны при первом рендере.
4. **Предзагрузка чанка.** Префетч маршрута протокола при наведении/фокусе на ссылку в журнале визитов (`src/pages/AdminPatientVisits.tsx`), по аналогии с `src/lib/prefetchAdmin.ts`.

**Ожидаемый эффект:** первое открытие протокола — минус тяжёлые библиотеки в чанке и минус один последовательный round-trip; повторные переходы почти мгновенные за счёт префетча.
